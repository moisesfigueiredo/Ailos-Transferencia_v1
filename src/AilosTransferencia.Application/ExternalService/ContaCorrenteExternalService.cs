...
namespace AilosTransferencia.Application.ExternalService
{
    public class ContaCorrenteExternalService
    {
        ...
        public async Task<ServiceResult?> GetContaCorrenteCadastradaAtiva(int numero, string accessToken)
        {
        ...
        public async Task<ServiceResult?> MovimentacaoContaCorrente(int? numContaCorrenteMovimento, decimal valor, string tipoMovimento, string accessToken)
        {
            var baseAddress = "http://ailos-contacorrente-api:8080";
            ...
            try
            {
                ...
                HttpResponseMessage response = await _httpClient.PostAsync($"{baseAddress}/ContaCorrente/MovimentacaoContaCorrente?numContaCorrenteMovimento={numContaCorrenteMovimento}&valor={valor}&tipoMovimento={tipoMovimento}", content);
        ...
        public async Task<ServiceResult> VerificaContaDestino(string tokenRequisicao, int numeroContaDestino)
        {
            ...
            try
            {
                ...
                var resultadoRequisicao = await GetContaCorrenteCadastradaAtiva(numeroContaDestino, tokenRequisicao);
